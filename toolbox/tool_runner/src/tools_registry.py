...
class ToolsRegistry:
    ...
    def load(cls, progress_callback=None):
        ...
        del cls._tools[:]
        files = []
        progress_callback(f'Load tools from: {AppData.TOOLS_PATH}')
        for current_path, subfolders, filenames in os.walk(AppData.TOOLS_PATH):
            if '__pycache__' in current_path:
                continue
            if not AppData.DEBUG and 'tool_template' in current_path:
                continue
            subfolders.sort()
        ...
        total = len(files)
        progress_callback(f'Load {total} tools')
        if total > 0:
            i = 0
            exceptions = []
            for i, filename in enumerate(files):
                rel_path = filename[len(AppData.TOOLS_PATH) + 1:]
                progress_callback(f'Load tool from: {rel_path} ({i + 1}/{total})')
