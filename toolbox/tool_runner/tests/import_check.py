...
def check_imports():
    packages = ('tools', 'tool_runner')
    n_cross_imports = 0
    for cross_name in packages:
        package = [ p for p in packages if p != cross_name ][0]
        cross_name = f'toolbox.{cross_name}'
        print(f'Check for imports from: {package}')
        files = _get_files(package)
        for file in files:
